...
class DeploymentV2LowMemoryActivationTest extends TestCase
{
    ...
    public function test_low_memory_cutover_has_explicit_locked_state_machine_and_maintenance_page(): void
    {
        ...
        $this->assertStringContainsString('v2_validate_rendered_production_compose()', $common);
        $this->assertStringContainsString('v2_validate_rendered_production_compose', $prepare);
        ...
        $migrationApproval = strpos($start, 'validate-approved-migrations.php');
        $migrationRun = strpos($start, 'php /www/artisan migrate --force --no-interaction');
        $migrationClean = strpos($start, 'validate-approved-migrations.php --require-clean');
        $databaseIntegrity = strpos($start, "PRAGMA integrity_check");
        $this->assertNotFalse($maintenanceSwitch);
        $this->assertNotFalse($legacyStop);
        $this->assertNotFalse($v2Start);
        $this->assertNotFalse($migrationApproval);
        $this->assertNotFalse($migrationRun);
        $this->assertNotFalse($migrationClean);
        $this->assertNotFalse($databaseIntegrity);
        $this->assertLessThan($legacyStop, $maintenanceSwitch);
        $this->assertLessThan($v2Start, $legacyStop);
        $this->assertLessThan($migrationApproval, $v2Start);
        $this->assertLessThan($migrationRun, $migrationApproval);
        $this->assertLessThan($migrationClean, $migrationRun);
        $this->assertLessThan($databaseIntegrity, $migrationClean);
        $this->assertStringContainsString('database_backup_sha256', $common);
        ...
        $candidateCleanup = strpos($rollbackImplementation, 'v2_cleanup_candidate_runtime');
        ...
        $this->assertNotFalse($candidateCleanup);
        ...
        $this->assertLessThan($restoreRedisOwner, $candidateCleanup);
        ...
        $this->assertStringContainsString('candidate_compose_down_failed', $common);
        $this->assertStringContainsString('--remove-orphans --timeout 60', $common);
        $this->assertStringNotContainsString('v2_compose down --volumes', $common);
        $this->assertStringContainsString('candidate_cleanup_required true', $common);
        $this->assertStringContainsString('candidate_cleanup_required false', $common);
        $this->assertStringContainsString('candidate_cleanup_required', $rollback);
        $this->assertStringContainsString('candidate_runtime_cleanup_retry_failed', $rollback);
